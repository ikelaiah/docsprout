  ...
  checks:
    ...
    steps:
      ...
      - name: Install DocSprout
        run: python -m pip install "https://github.com/ikelaiah/docsprout/archive/refs/tags/v1.1.5.zip"
...
Pin a released tag (`v1.1.5` above), never a branch. `--format json` writes the
...
docs-checks:
  image: python:3.12
  script:
    - python -m pip install "https://github.com/ikelaiah/docsprout/archive/refs/tags/v1.1.5.zip"
    - docsprout check
